...
Loop 校正后需要更细分地写 diffusion：DiAD 这类 multi-class diffusion 的核心矛盾是类别保持与像素结构完整性；AnomalyDiffusion/DefectFill 这类异常生成工作的核心矛盾是真实性、mask alignment 和少样本缺陷分布；OmiAD 这类 one-step diffusion 的核心矛盾是迭代推理成本；GLAD 这类 adaptive diffusion 的核心矛盾是不同样本和局部异常区域的难度不均。不要把所有 diffusion limitation 简化成“慢”或“重建不准”。
...
Loop 校正后要补一条原则：synthetic anomaly 不是普通数据增强，而是 DRAEM、DeSTSeg、SimpleNet 等方法建立 feature/pixel-level decision boundary 的监督来源。我们可以说 normality-flow 不依赖 synthetic anomaly 的真实感或 mask 对齐，但不能暗示 synthetic anomaly 路线价值低。
...
Loop 校正后，这里需要加入 CFLOW-style hard check：CFLOW-AD 的真实写法不是“用了 flow”这么简单，而是 normal-only setting 下用 discriminatively pretrained encoder 与 multi-scale conditional generative decoders 显式估计 encoded features likelihood，并把 real-time speed 和 memory efficiency 作为证据。因此我们的区别句必须回答四件事：训练目标是否不同、inference anomaly score 如何构造、multi-scale/local deviation 是否不同、runtime/memory 是否有净收益。
...
| Generalization / harder setting | 证明不是只吃 MVTec AD | VisA、BTAD、Real-IAD、MVTec AD2 或跨类别设置是否成立；若声称 real-world，还要报告 sample-level、threshold stability 或 setting-level 指标 |
| Efficiency and cost | 证明实用性 | FPS、latency、参数量、显存、memory bank size、diffusion steps 是否有优势 |
| Score construction and calibration | 证明异常分数不是拍脑袋 | likelihood/deviation/anomaly map 如何从 normality-flow 输出得到，是否比 CFLOW/FastFlow 的 likelihood scoring 有可验证差异 |
...
7. Flow-score construction comparison，特别是与 conditional flow likelihood scoring、raw feature distance、nearest-neighbor distance 的差异。
...
| 可能攻击 | 为什么危险 | 写作/实验防线 |
|---|---|---|
| “Flow-based AD 已经有 CFLOW/FastFlow，你的新意在哪里？” | 这是最直接拒稿点 | Related Work 单独列 flow/density；明确 training objective、inference score、multi-scale/local deviation 与 CFLOW-style likelihood estimation 的 exact distinction；ablation 证明该 distinction 有效 |
| “只是换了特征或 backbone” | 会被判为 incremental | 做不同 backbone 实验；把贡献写在 normality formulation 和 local scoring，而不是 backbone |
| “MVTec AD 已饱和” | 结果很难说服 top venue | 加 VisA、BTAD、Real-IAD、MVTec AD2 或明确 harder setting；引用 Real-IAD 的 benchmark-saturation 逻辑；若写 real-world/practical，还要有 sample-level metric、threshold stability 或 FUIAD/工业设定解释 |
...
我用四个同主题 loop 对照已发表论文后，对本学习结果做了定点修订：

1. PatchCore + CFLOW-AD loop：确认 normal-only / feature-space / localization 方向一致，但加强了 flow direct competitor 的 hard check，避免把 CFLOW/FastFlow 只当名字列出。
2. DRAEM + SimpleNet + DeSTSeg loop：确认 synthetic/discriminative 路线理解基本正确，但补充了 synthetic anomaly 的监督价值，避免把它轻写成普通增强或弱路线。
3. DiAD loop：确认 multi-class diffusion 的核心不是泛泛“diffusion 慢”，而是 category preservation 与 pixel-wise structural integrity；已把 diffusion limitation 分层。
4. Real-IAD loop：确认 benchmark saturation 和 public-real gap 判断正确，但补充了 sample-level metric、threshold stability 和 setting-level justification。

修订后，这份学习结果的质量标准更接近 top 论文真实写法：同一 theme 下，必须能复现 published paper 的 constraint -> failure -> method -> evidence -> claim boundary 路线，而不是只复述方法名。

# 11. Source links checked
