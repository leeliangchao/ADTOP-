...
Supervisor-Skills 融合使用入口：如果要把本文件真正用于写作，不要只读下面的论文套路总结；应先按 `异常检测论文写作操作手册_融合版.md` 走一遍 paper type positioning、thinking template、Introduction flowchart、running example、Figure 1、contribution-section mapping 和 pre-submission audit。该融合流程把本文件中的 AD 领域经验转化为可执行的写作工作流。
